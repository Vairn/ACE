...
void bitmapSave(const tBitMap *pBitMap, const char *szPath) {
	...
	// Data
	if(ubFlags & BITMAP_INTERLEAVED) {
		fileWriteBytes(pFile, pBitMap->Planes[0], (uwWidth >> 3) * uwHeight * ubPlaneCount);
	}
	else {
		for (FUBYTE i = 0; i != ubPlaneCount; ++i) {
			fileWriteBytes(pFile, pBitMap->Planes[i], (uwWidth >> 3) * uwHeight);
		}
	}
...
void bitmapSaveBmp(
	...
	// BMP header
	fileWriteBytes(pOut, (const UBYTE*)"BM", 2);
